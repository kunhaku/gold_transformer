...
class TransformerEncoder(Layer):
    ...
    def call(self, inputs, mask=None, training=False):
        ...
        if mask is not None:
            ...
            mask = tf.expand_dims(mask, axis=1)     # (batch_size, 1, 1, seq_length)
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for g in unique_groups:
            # 取該 group 的樣本 index (時間順序)
            idxs = np.where(group_ids == g)[0]
            # (若有需要，可根據實際「逐步加入未來資訊」的邏輯來排序 idxs)
            # 例如 idxs = sorted(idxs)

            # 避免上一個 group 的預測影響到這個 group
            # 初始化 past_preds=0 (batch=1, forecast_length)
            past_preds = tf.zeros((1, forecast_length), dtype=tf.float32)

            for i in idxs:
                # 取出單筆
                x_i = X_data[i]       # (seq_len, input_dim)
                y_i = y_data[i]       # (forecast_length,)
                m_i = mask_data[i]    # (seq_len,)

                # 增加 batch 維度
                x_i = x_i[None, ...]  # (1, seq_len, input_dim)
                y_i = y_i[None, ...]  # (1, forecast_length)
                m_i = m_i[None, ...]  # (1, seq_len)

                # 執行單筆 train_step
                preds, loss_value = train_step(model, optimizer, loss_fn, x_i, m_i, y_i, past_preds)

                # 更新 past_preds:
                # 變成 (1, forecast_length) 以便下一筆繼續用
                past_preds = preds  # preds 本來 shape=(1, forecast_length)
